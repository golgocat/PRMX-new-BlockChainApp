//! PRMX Node Service
//!
//! Defines the node service configuration including consensus and networking.

use futures::FutureExt;
use prmx_runtime::opaque::Block;
...
/// Create a new partial node with full state.
...
> {
    ...
    let (client, backend, keystore_container, task_manager) =
        sc_service::new_full_parts::<Block, prmx_runtime::RuntimeApi, _>(
            config,
    ...
    let transaction_pool = sc_transaction_pool::BasicPool::new_full(
        Default::default(), // config.transaction_pool.clone().into(),
...
/// Builds a new full node service.
pub fn new_full(config: Configuration) -> Result<TaskManager, ServiceError> {
    ...
    let (network, system_rpc_tx, tx_handler_controller, sync_service) =
        sc_service::build_network(sc_service::BuildNetworkParams {
            ...
            warp_sync_config: None, // WarpSyncParams::WithProvider(warp_sync),
            block_relay: None,
    ...
    let rpc_extensions_builder = {
        ...
        Box::new(move |_subscription_executor| {
             let deny_unsafe = sc_rpc::DenyUnsafe::Yes; // safe default
