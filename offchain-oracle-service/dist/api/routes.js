 ...
 */
import { getMonitors, getBuckets, clearAllData, getObservationsV3, getSnapshotsV3, checkDatabaseHealth } from '../db/mongo.js';
import { runEvaluationCycle } from '../scheduler/monitor.js';
import { evaluateMonitor } from '../evaluator/cumulative.js';
import { fetchCurrentConditions, fetchHistorical24Hours } from '../accuweather/fetcher.js';
...
export function setupRoutes(app) {
    ...
    // =========================================================================
    // Get all monitors
    app.get('/monitoring/monitors', async (req, res) => {
        try {
    ...
    app.get('/monitoring/monitors/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const monitors = getMonitors();
    ...
    app.get('/monitoring/monitors/:id/buckets', async (req, res) => {
        try {
    ...
    app.post('/monitoring/monitors/:id/backfill', async (req, res) => {
        try {
            const { id } = req.params; // Format: "market_id:policy_id"
            const monitors = getMonitors();
            const buckets = getBuckets();
            ...
            for (const missing of missingHours) {
                ...
                if (historical) {
                    ...
                    await buckets.updateOne({ _id: missing.bucketId }, {
                        $set: {
                            monitor_id: id,
                            hour_utc: hourUtc,
                            mm: historical.mm,
    ...
    app.post('/monitoring/monitors/trigger-all', async (req, res) => {
        try {
    ...
    app.post('/monitoring/monitors/:id/trigger', async (req, res) => {
        try {
            const { id } = req.params; // Format: "market_id:policy_id"
            const monitors = getMonitors();
            const monitor = await monitors.findOne({ _id: id });
    ...
    app.post('/monitoring/admin/clear-database', async (req, res) => {
        try {
    ...
    app.get('/monitoring/test/accuweather/:locationKey', async (req, res) => {
        try {
    ...
    app.post('/monitoring/admin/monitors/:id/reset', async (req, res) => {
        try {
            const { id } = req.params;
            const monitors = getMonitors();
            const monitor = await monitors.findOne({ _id: id });
            ...
            await monitors.updateOne({ _id: id }, {
                $set: {
                    state: 'monitoring',
                    updated_at: new Date(),
                },
    ...
    const usedNonces = new Map(); // In production, use Redis
     ...
     */
    function logV3Request(req, endpoint, success) {
        if (!V3_REQUEST_LOGGING)
            return;
        const ip = req.ip || req.socket.remoteAddress || 'unknown';
     ...
     */
    function validateV3Signature(req) {
     ...
     */
    function computeBlake2Signature(input) {
     ...
     */
    app.post('/ingest/observations/batch', async (req, res) => {
        try {
            // Rate limiting
            ...
            for (const sample of samples) {
                ...
                const fields = {};
                if (sample.precip_1h_mm_x1000 !== undefined)
                    fields.precip_1h_mm_x1000 = sample.precip_1h_mm_x1000;
                if (sample.temp_c_x1000 !== undefined)
                    fields.temp_c_x1000 = sample.temp_c_x1000;
                if (sample.wind_gust_mps_x1000 !== undefined)
                    fields.wind_gust_mps_x1000 = sample.wind_gust_mps_x1000;
                if (sample.precip_type_mask !== undefined)
                    fields.precip_type_mask = sample.precip_type_mask;
                // Also support normalized_fields from test tools
                if (sample.normalized_fields)
                    Object.assign(fields, sample.normalized_fields);
                try {
                    const result = await observations.updateOne({ _id: docId }, {
                        $setOnInsert: {
                            policy_id,
     ...
     */
    app.post('/ingest/snapshots', async (req, res) => {
        try {
            // Rate limiting
            const rateLimitResult = checkRateLimit(req);
            if (!rateLimitResult.allowed) {
            ...
            const result = await snapshots.updateOne({ _id: docId }, {
                $setOnInsert: {
                    policy_id,
                    observed_until,
    ...
    app.get('/admin/health', async (req, res) => {
        ...
        try {
            ...
            const v3Monitored = services.oracle_v3.policies_monitored || 0;
            ...
            const [lastSnapshot, lastObservation] = await Promise.all([
                snapshots.findOne({}, { sort: { inserted_at: -1 } }),
                observations.findOne({}, { sort: { inserted_at: -1 } }),
            ]);
